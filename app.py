...
def get_token(key, secret):
    ...
    try:
        ...
        # API 응답 구조에 따라 토큰 위치 탐색
        token = res.get('access_token')
        if not token and 'data' in res:
...
if st.sidebar.button("기관 데이터 동기화 🔄"):
    ...
    if token:
        with st.spinner('데이터를 정밀 분석 중입니다...'):
            ...
            if not df_m.empty:
                # 1. [핵심] 그룹(소속기관) 필드 안전하게 찾기
                # group_name, group, groups 중 있는 것을 소속기관으로 설정
                if 'group_name' in df_m.columns:
                    df_m['소속기관'] = df_m['group_name'].fillna('미지정(일반)')
                elif 'group' in df_m.columns:
                    df_m['소속기관'] = df_m['group'].fillna('미지정(일반)')
                else:
                    df_m['소속기관'] = '미지정(일반)'
                
                # 2. 이름 및 가입일 정리
                df_m['이름'] = df_m['name'] if 'name' in df_m.columns else (df_m['nickname'] if 'nickname' in df_m.columns else '이름없음')
                ...
                if not df_o.empty:
                    def parse_order(row):
                        items = row.get('items', [])
                        prod_name = items[0].get('prod_name') if items else "정보없음"
                        pay = row.get('payment', {})
                        return pd.Series({
                ...
                if selected_group != "전체" and not view_df[view_df['신청강좌'] != '미신청'].empty:
                    ...
                    fig = px.bar(course_data, x='count', y='신청강좌', orientation='h', color='신청강좌')
                    st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("데이터가 비어있습니다.")
    else:
        st.error("API 토큰을 가져올 수 없습니다.")
