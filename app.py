...
if file_m and file_o:
    ...
    if df_m_raw is not None and df_o_raw is not None:
        ...
        # 가져올 회원 컬럼 리스트 (구매횟수 -> 강좌 신청 횟수 변경 포함)
        m_cols_to_fetch = [
        ...
        # Master 주문 내역을 기준으로 회원 정보 병합
        df_final = pd.merge(
            df_master, 
            df_m_subset.drop(columns=['이메일']), # 중복 이메일 방지
            on='match_key', 
            how='left'
        )
        ...
        st.success(f"✅ 통합 완료: 주문 내역 {len(df_master)}건 기준 매칭 성공")
        
        # 데이터 출력
        st.subheader("📋 통합 데이터 리스트")
        ...
        st.download_button(
            ...
            data=csv,
            file_name="LearnIQ_Integrated_Report.csv",
            mime="text/csv"
        )
else:
    st.info("💡 사이드바에 주문 내역(Master)과 회원 목록 파일을 업로드해 주세요.")
